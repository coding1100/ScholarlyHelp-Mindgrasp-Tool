...
async function fetchOnlineClassData() {
  try {
    ...
    console.log('Querying online_classes collection with query:', JSON.stringify(query));
    ...
    if (!content) {
      const allDocs = await db.collection('online_classes').find({}).limit(5).toArray();
      console.log('Sample documents in online_classes:', allDocs.map(d => ({ id: d.id, slug: d.slug })));
    }
  ...
  try {
    ...
    if (databaseUrl) {
      ...
      if (pageData) {
        ...
        const metaDescription = pageData.meta?.description || MetaData.onlineClass.description;
        const canonicalUrl = pageData.meta?.canonicalUrl || `${baseUrl}${MetaData.onlineClass.url}`;
