...
async function fetchEssayWritingData() {
  try {
    ...
    console.log('Querying essay_writing collection with query:', JSON.stringify(query));
    ...
    if (!content) {
      const allDocs = await db.collection('essay_writing').find({}).limit(5).toArray();
      console.log('Sample documents in essay_writing:', allDocs.map(d => ({ id: d.id, slug: d.slug })));
    }
  ...
  try {
    ...
    if (databaseUrl) {
      ...
      if (pageData) {
        ...
        const metaDescription = pageData.meta?.description || MetaData.essayWriting.description;
        const canonicalUrl = pageData.meta?.canonicalUrl || `${baseUrl}${MetaData.essayWriting.url}`;
