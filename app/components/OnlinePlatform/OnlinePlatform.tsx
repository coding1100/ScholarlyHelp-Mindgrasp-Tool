...
export default function OnlinePlatform() {
  const pageData = usePageData() as { onlinePlatform?: { mainHeading?: string; subHeading?: string; platforms?: { key: string; name?: string; description?: string }[] } } | null;
  const adminSection = pageData?.onlinePlatform;

  const { mainHeading, subHeading, platforms } = useMemo(() => {
    const mainHeading =
    ...
    const adminPlatforms = Array.isArray(adminSection?.platforms)
      ...
      : [];
    const platforms = DEFAULT_PLATFORMS.map((def) => {
      const adminPlat = adminPlatforms.find(
        (p: { key?: string }) => p?.key === def.key
      );
      return {
        ...def,
        name: (adminPlat?.name && String(adminPlat.name).trim()) || def.name,
        description:
          (adminPlat?.description && String(adminPlat.description).trim()) ||
          def.description,
      };
    });
    return { mainHeading, subHeading, platforms };
  }, [adminSection]);
