...
function TourTooltipFactory(onEndTour: () => void) {
  return function AcademicResearchTourTooltip(props: TooltipRenderProps) {
    const {
      backProps,
      closeProps,
      continuous,
...
type AcademicResearchAssistantTourProps = {
  /** True when the full document editor is shown (vs. welcome / setup screen). */
  tourEditorActive: boolean;
};

const AcademicResearchAssistantTour: React.FC<
  AcademicResearchAssistantTourProps
> = ({ tourEditorActive }) => {
  const pathname = usePathname();
  const normalized = normalizePath(pathname);
  const isAssistantRoute =
    normalized === "/tools/academic-research-assistant";
  ...
  const finalizeTour = useCallback(async () => {
    ...
    try {
      await completeAcademicResearchTour();
  ...
  useEffect(() => {
    ...
    getAcademicResearchTourState()
      .then((state) => {
        if (cancelled) return;
        const done = !!state?.academic_research_assistant_tour_completed;
  ...
  if (!isAssistantRoute || loading || tourCompleted) {
    return null;
  }
...
export default AcademicResearchAssistantTour;
