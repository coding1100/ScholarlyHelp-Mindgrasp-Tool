...
export default function Step1({ onContinue }: Step1Props) {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (knowledgeLevelRef.current && !knowledgeLevelRef.current.contains(event.target as Node)) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : "Failed to create study schedule. Please try again.";
                ...
                <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-8 md:p-12 shadow-2xl">
                    ...
                    <div className="text-center mb-4">
                        ...
                        <p className="text-black text-sm mt-2">
                            Tell us about the exam you're preparing for<br />so we can create a personalized study plan.
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="relative" ref={examTypeRef}>
                                <button
                                    ...
                                    className="w-full text-sm px-4 py-3 rounded-lg bg-gray-100 backdrop-blur-md border border-gray-300/50 text-black text-left flex items-center justify-between focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
                                >
                                ...
                                {isExamTypeOpen && (
                                    <div className="absolute z-50 w-full mt-2 bg-blue-600 rounded-lg shadow-xl border border-blue-500/50 max-h-60 overflow-y-auto">
                                        {examTypes.map((type, index) => (
                                            <button
                                                ...
                                                className={`w-full px-4 py-3 text-left text-white hover:bg-blue-500 transition-colors ${examType === type ? "bg-blue-500" : ""
                        ...
                        <div>
                            ...
                            <div className="relative" ref={knowledgeLevelRef}>
                                <button
                                    ...
                                    className="w-full text-sm px-4 py-3 rounded-lg bg-gray-100 backdrop-blur-md border border-gray-300/50 text-black text-left flex items-center justify-between focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
                                >
                                    <span className={knowledgeLevel ? "text-black" : "text-gray-400"}>
                                    ...
                                    <div className="absolute z-50 w-full mt-2 bg-blue-600 rounded-lg shadow-xl border border-blue-500/50 max-h-60 overflow-y-auto">
                                        {knowledgeLevels.map((level, index) => (
                                            <button
                                                ...
                                                className={`w-full px-4 py-3 text-left text-white hover:bg-blue-500 transition-colors ${knowledgeLevel === level ? "bg-blue-500" : ""
                                                    } ${index === 0 ? "text-gray-300 cursor-not-allowed" : ""}`}
                                                disabled={index === 0}
                                            >
                                                {level}
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 rounded-lg bg-white/40 backdrop-blur-md border border-gray-300/50 text-black placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all text-sm italic"
                            />
                        </div>
                        ...
                        {/* Error Message */}
                        {error && (
                            <div className="px-4 py-3 rounded-xl bg-red-300 border border-red-500 text-white text-sm">
                                {error}
                            </div>
                        )}
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="w-full py-3 rounded-lg bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-semibold text-sm transition-all duration-200 shadow-lg hover:shadow-xl active:scale-[0.98] cursor-pointer gap-2 flex items-center justify-center"
                        >
                            {isLoading ? (
                                <>
