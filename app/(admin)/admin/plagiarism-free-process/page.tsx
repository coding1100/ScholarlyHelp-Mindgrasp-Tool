...
export default function PlagiarismFreeProcessAdmin() {
  ...
  useEffect(() => {
    const loadPage = async () => {
      setPageLoading(true);
      try {
        const res = await fetch(`/api/admin/plagiarism-free-process`);
  ...
  const handlePageSave = async () => {
    ...
    setPageLoading(true);
    try {
      const response = await fetch('/api/admin/plagiarism-free-process', {
        method: 'POST',
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div>
              ...
              {(pageData.originalityContent?.steps || []).map((step: any, index: number) => (
                <div key={index} className="mb-4 p-4 border border-gray-200 rounded-md">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button
                      type="button"
                      onClick={() => removeArrayItem('originalityContent.steps', index)}
              ...
              <button
                type="button"
                onClick={() => addArrayItem('originalityContent.steps', { title: '', description: '' })}
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Edit Plagiarism-Free Process Page</h1>
        <p className="mt-2 text-sm text-gray-600">Update the content for the Plagiarism-Free Process page</p>
      </div>
      {renderPageForm()}
    </div>
  );
