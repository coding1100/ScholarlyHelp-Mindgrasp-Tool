...
export default function UsBasedPhdExpertsAdmin() {
  ...
  useEffect(() => {
    const loadPage = async () => {
      setPageLoading(true);
      try {
        const res = await fetch(`/api/admin/us-based-phd-experts`);
  ...
  const handlePageSave = async () => {
    ...
    setPageLoading(true);
    try {
      const response = await fetch('/api/admin/us-based-phd-experts', {
        method: 'POST',
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Edit US-Based PhD Experts Page</h1>
        <p className="mt-2 text-sm text-gray-600">Update the content for the US-Based PhD Experts page</p>
      </div>
      {renderPageForm()}
    </div>
  );
